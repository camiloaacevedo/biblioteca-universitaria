  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="flex items-center gap-3 mb-6">
        <Link href="/libros" className="text-gray-500 hover:text-gray-700">← Volver</Link>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-2 bg-white rounded-xl shadow p-6">
          <div className="flex justify-between items-center mb-4">
            ...
            <Link href={`/ejemplares/nuevo?libro_id=${libro.id}`}
          ...
          {libro.ejemplares?.length === 0 ? (
            <p className="text-gray-400 text-center py-8">No hay ejemplares registrados</p>
          ) : (
              ...
              <tbody className="divide-y divide-gray-100">
                {libro.ejemplares?.map(ej => (
                  <tr key={ej.id} className="hover:bg-gray-50">
                    <td className="px-3 py-2 font-mono text-xs">{ej.codigo_barras}</td>
                    <td className="px-3 py-2 text-gray-600">{ej.ubicacion || '—'}</td>
                    <td className="px-3 py-2">
                      <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                        ej.estado === 'disponible' ? 'bg-green-100 text-green-700' :
                        ej.estado === 'prestado' ? 'bg-yellow-100 text-yellow-700' :
