...
export default async function UsuariosPage({ searchParams }) {
  ...
  let query = supabase.from('usuarios').select('*').order('nombres');

  if (busqueda) {
    query = query.or(
      `nombres.ilike.%${busqueda}%,correo.ilike.%${busqueda}%,codigo.ilike.%${busqueda}%`,
    );
  }
    ...
    <div>
      ...
      <form method="GET" className="mb-6 flex gap-3">
        ...
        >
          ...
          <option value="estudiante" selected={filtroRol === 'estudiante'}>
            Estudiante
          </option>
          <option value="docente" selected={filtroRol === 'docente'}>
            Docente
          </option>
          <option
            value="bibliotecario"
            selected={filtroRol === 'bibliotecario'}
          >
            Bibliotecario
          </option>
        </select>
