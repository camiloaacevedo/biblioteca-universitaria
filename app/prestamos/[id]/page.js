...
import { registrarDevolucionAction, ampliarPrestamoAction } from '../actions';
...
export default async function DetallePrestamoPage({ params }) {
  ...
  const vencimiento = prestamo.fecha_vencimiento
    ...
    <div className="max-w-2xl mx-auto">
      ...
      <div className="bg-white rounded-xl shadow p-6 space-y-4 mb-6">
        <div className="flex justify-between items-center border-b pb-3">
          ...
          <span
            className={`px-3 py-1 rounded-full text-sm font-medium ${coloresEstado[prestamo.estado]}`}
          >
            {prestamo.estado}
          </span>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4 text-sm">
          ...
          <div>
            ...
            <p className="text-gray-400 font-mono text-xs">
              {prestamo.ejemplares?.libros?.isbn}
            </p>
          </div>
          <div>
      ...
      {/* Ampliar préstamo — solo si está aprobado y no ha sido ampliado */}
      {esAdmin && prestamo.estado === 'aprobado' && !prestamo.ampliado && (
        <div className="bg-white rounded-xl shadow p-6 mb-6">
          <h2 className="font-bold text-gray-700 mb-4">📅 Ampliar Préstamo</h2>
          <p className="text-sm text-gray-500 mb-4">
            Extiende la fecha de vencimiento por el mismo periodo del préstamo
            original. Solo se puede ampliar una vez y solo si no hay reservas
            activas.
          </p>
          <form action={ampliarPrestamoAction}>
            <input type="hidden" name="id" value={prestamo.id} />
            <input
              type="hidden"
              name="ejemplar_id"
              value={prestamo.ejemplar_id}
            />
            <button
              type="submit"
      ...
      {prestamo.ampliado && (
        <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-4 mb-6">
          <p className="text-yellow-700 text-sm">
            ✅ Este préstamo fue ampliado el{' '}
            {new Date(prestamo.fecha_ampliacion).toLocaleDateString('es-CO')}
          </p>
        </div>
      )}
          ...
          {estaVencido && (
            <div className="bg-red-50 border border-red-200 rounded-lg p-4 mb-4">
              ...
              <p className="text-red-500 text-xs mt-1">
                Tarifa: ${config?.tarifa_multa_dia?.toLocaleString('es-CO')} por
                día
              </p>
            </div>
          )}
          ...
          <form action={registrarDevolucionAction} className="space-y-4">
            ...
            <div>
              ...
              >
                ...
                <option value="cubierta deteriorada">
